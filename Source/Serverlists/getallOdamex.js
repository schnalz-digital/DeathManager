const {getServerListFromMaster} = require("./getServerlistOdamex.js");
const {getServerInfos} = require("./getServerInfosOdamex.js");
...
myEmitter.on('send', async (data) => {
    ...
    for (let i=0; i < data.length; i++) {
        ...
        if (sum == servers.length) {
            let withanswer = servers.filter(e=>e.info?.map != undefined);
