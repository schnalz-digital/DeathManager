const {getServerListFromMaster} = require("./getServerlistChoc.js");
const {getServerInfos} = require("./getServerInfosChoc.js");
...
myEmitter.on('send', async (data) => {
    ...
    for (let i=0; i < data.length; i++) {
        ...
        if (sum == servers.length) {
            let withanswer = servers.filter(e=>e.info.version != undefined);
